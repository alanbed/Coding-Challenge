...
class PostController {
  static async escapeHtml(unsafe) {
    return unsafe
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;')
  ...
  static async insertOne(newPost) {
    try {
      ...
      if (postBlob) {
        postBlob.post = await PostController.escapeHtml(postBlob.post);
        const post = new Post(postBlob);
        console.log('pb: ', postBlob);
        return Common.GenerateResult.Ok(post);
      }
  ...
  static async getAll(orderDirection, orderBy, postCountLimit, pageNumber) {
    try {
      const postsPaginated = await PostDb.getAll(
        orderDirection,
        orderBy,
        postCountLimit,
        pageNumber
      ...
      if (postsPaginated) {
        const postsMapped = postsPaginated.posts.map((p) => {
          return new Post(p);
        });
